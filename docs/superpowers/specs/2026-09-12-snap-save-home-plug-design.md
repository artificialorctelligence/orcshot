...
- **Not proven live yet:** that the write is a `PermissionError` rather than silent. Stated as
  the mechanism it must be; Scenario 4 below observes it before the fix on the VM (the
  unfixed 0.3.0 CI snap) so the entry's guess is replaced by a fact.
...
snap-specific change; sections 2 and 3 make them correct there.
...
- **Before the fix (the 0.3.0 CI snap, or this branch's snap with `home` disconnected via
  `snap disconnect orcshot:home`):** *Capture Full Screen → Save*. Record what actually
  happens — expected `PermissionError` in `snap run` output, no file. This replaces #212's
  "presumably".
- **A. Default folder.** `home` connected (`snap connections orcshot | grep home`), no
  `output_directory` configured. *Save*: no dialog; file in `~/Pictures/Screenshots` on the VM.

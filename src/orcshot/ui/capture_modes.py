...
from orcshot.settings import get_capture_mouse_cursor


def _default_window_enumerator() -> WindowEnumerator:
    from orcshot.capture.backend_select import default_window_enumerator_and_activator
...
def _capture_and_pick(
    ...
    if os.environ.get("XDG_SESSION_TYPE") == "wayland":
        from orcshot.capture.gnome_capture_rect import is_available as gnome_shell_capture_available

        if gnome_shell_capture_available():
            from orcshot.capture.gnome_capture_rect import start_capture_rect
...
def start_full_screen_capture(
    ...
    the picker opens - OrcshotApplication uses this to remember the

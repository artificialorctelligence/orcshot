...
LOGO_PATH: the real logo image (not a recreation) - used for both the
window/taskbar icon (Gtk.Window.set_default_icon_from_file, see
app.py's do_startup) and the system tray icon
(Gtk.StatusIcon.set_from_file, see _build_tray_icon), and referenced
by the autostart .desktop entry's Icon field (see autostart.py). One
shared asset for every icon surface, rather than a separate

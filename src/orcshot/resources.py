...
asset instead: the GNOME Shell extension and the XApp status icon
(ui/xapp_tray.py) reference it by icon-theme name (Gio.ThemedIcon.new(
"orcshot")/XApp.StatusIcon.set_icon_name("orcshot")), resolving to
usr/share/icons/hicolor/128x128/apps/orcshot.png via
debian/orcshot.install - not this module's LOGO_PATH, since the extension
runs in a separate process (GNOME Shell) that can't read this

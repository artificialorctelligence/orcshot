...
在 Wayland 桌面上，Orcshot 托盘图标的菜单是由一个独立组件(一个 GNOME Shell 扩展)而不是主程序绘制的——但它复用的是完全相同的翻译。您不需要做任何不同的事情，也不需要填写第二个文件：翻译 `po/<lang>.po` 会自动覆盖托盘菜单，无论是在 X11 还是 Wayland 上。
